...
##### 구조 구성 (Struct Composition)

프로토타이핑은 개념 증명서 작성과 구체적인 문제 해결 만큼이나 중요하다. 그러면 우리는 스스로에게 다음과 같이 물어볼 수 있다. 무엇이 변할 수 있을까? 어떤 변화가 오고 있는가? 디커플링과 리팩터(refactor)를 시작할 수 있도록 리팩토링(Refactoring)은 개발 주기의 일부가 되어야 한다.

여기 우리가 이 섹션에서 해결하려고 하는 문제가 있다. 우리 데이터베이스를 가진 `Xenia` 라는 시스템을 가지고 있다. `Pillar`라는 또 다른 시스템이 있는데, 그것은 `Xenia`를 소비하는 프론트엔드 웹 서버 이다. `Pillar` 또한 데이터베이스를 가지고 있다. 우리의 목표는 `Xenia`의 데이터를 `Pillar`에 옮기는 것이다.

얼마나 걸릴까? 하나의 코드 조각이 완성되면 다음 코드 조각으로 이동 할 수 있는지 어떻게 알 수 있는가? 만약 너가 기술책임자라면, 너의 부채(debt)가 "눈물나는 노력" 인지, "충분히 노력하지 않은 것" 인지 어떻게 알 수 있을까?

대답하자면, 완성되는 것 두 파트로 구성된다. 첫번째는 테스트 커버리지이다. 80%의 커버리지는 일반적이고 100% 커버리지는 완벽하다. 두번째는 변화에 관한 것이다. 기술적 관점과 사업적 관점에서 무엇이 변할 수 있는지 물어 봄으로써, 우리는 그 변화를 다룰 수 있도록 코드를 리팩터링 할 수 있다.

한 예로, 우리는 당신에게 구체적인 버전을 이틀 안에 줄 수 있지만, 변화에 대처하기 위한 코드 리팩터링을 2주간 해야한다. 한번에 한 문제씩 해결한다는 계획이다. 모든 것에 압도당하지 마라. 코드를 작성하고, 테스트와 리팩터를 만들어라. 각 계층이 다음을 위한 강력한 기초라는 것을 알고, 각자의 위에서 동작하는 API 계층을 만들어라.

구현 세부 사항에 너무 신경 쓰지마라. 여기서 중요한 것은 메커니즘(mechanics)이다. 우리는 성능이 아닌 정확성을 위해서 최적화 하고 있다. 속도가 충분히 빠르지 않다면 언제든 돌아갈 수 있다.
...
우리가 해결해야 할 첫 번째 문제는 타이머로 작동하는 소프트웨어가 필요하다는 것이다. `Xenia`에 접속해서, 데이터베이스를 읽고, 우리가 이동하지 않은 모든 데이터를 찾아내고, 그것을 추출해야 한다.
