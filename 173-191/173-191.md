...
Error: 정보를 출력하고 테스트 실패를 알리지만 본 테스트의 코드를 계속 실행함.
...
"go test" 명령어를 사용하면 테스팅 도구가 해당 함수를 찾습니다. "go test -v" 명령어로 로그의 전체 출력 결과를 확인할 수 있다. 만약 많은 테스트 함수가 있다면, "go test -run TestBaisc" 명령어는 `TestBasic` 함수는 테스트할 것이다.
