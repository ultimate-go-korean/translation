...
Benchmark를 하려면 Benchmark 함수를 가진 `<file_name>_test.go` 파일이 필요합니다. 이번에 알아볼 것은 Sprint와 Sprintf 중 어느 것이 더 성능이 좋은지, 자원 할당이 효율적인지 알아내는 것입니다. Sprint가 문자열 포맷을 하는 동안 오버헤드가 없기 때문에 더 나을 것이라고 추측하고 계신가요? 하지만, 그렇지 않습니다. 추측 보다는 실제로 확인된 결과를 통해 판단해야 한다는 것을 명십하십시오.
...
`BenchmarkSprintBasic`은 Sprint 성능을 테스트합니다. Benchmark 하려는 모든 코드는 `b.N` 반복문 안에 있어야 합니다. Benchmark 도구가 처음 호출 할 때 `b.N`은 1이고, 충분히 테스트가 진행될 때 까지 `b.N`값은 지속적으로 증가합니다. `fmt.Sprint`는 값을 반환하고, 우리는 이 값이 캡처되길 원하기 때문에 전역 변수 gs에 할당하여 \*dead code[^1] 처럼 보이지 않게 됩니다.
...
`BenchmarkSprintfBasic`은 Sprintf의 성능을 테스트합니다.
...
하위 테스트와 마찬가지로, 하위 Benchmark도 할 수 있습니다.
...
`BenchmarkSprintSub` 는 Sprint와 관련된 모든 하위 Benchmark를 합니다.
...
`benchSprint`는 Sprint의 성능을 테스트합니다.
...
`benchSprintf`는 Sprintf의 성능을 테스트합니다.
