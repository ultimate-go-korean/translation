...
스택 트레이스에서 볼 수 있는 `main.example (0xc420053f38, 0x2, 0x4, 0x1066c02, 0x5, 0xa)`에서 각각에 해당되는 값은 `slice의 포인터 주소, 2(길이), 4(용량), string의 포인터 주소, 5(길이), a(16진수로 표기된 10)` 이다.
...
Memory Tracing gives us a general idea if our software is healthy as related to the GC and memory in the heap that we are working with. Below is a sample program that causes memory leaks[^memory-leak].
...
당신이 The Ultimate Go Book을 잘 이해할 수 있었는지 이메일([hoanhan101@gmail.com](hoanhan101@gmail.com))로 알려주시길 바랍니다.
