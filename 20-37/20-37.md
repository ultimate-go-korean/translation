...
인덱스 0의 문자열은 이제 바이트들(문자열을 구성하는 문자들)을 실제로 저장하고 있는 배열에 대한 포인터와 길이 정보 5를 가지게 됩니다. 

##### 비용이 얼마나 드는가?(What is the cost?)

할당에는 2 바이트를 복사하는 비용이 발생합니다. 두 문자열은 같은 배열을 가리키며, 그래서 할당의 비용은 2 단어에 대한 비용뿐입니다.
...
range를 사용하면, 인덱스와 복사된 원소의 값을 얻을 수 있습니다.
for 문 내에서 fruit 변수는 문자열 값을 가지게 됩니다. 첫 번째 반복에서는 "Apple"을 가집니다. 이 문자열 역시 위 이미지의 (1) 배열을 가리키는 워드와 길이 5를 나타내는 두 번째 워드를 가집니다. 이제 세 개의 문자열의 같은 배열을 공유하고 있습니다.
...
4개의 정수를 가지는 배열을 선언하고 리터럴 표기법(literal syntax)으로 특정 값으로 초기화 합니다.
