...
문자열이 원소이고 길이가 5인 배열을 선언하고, 제로 값(zero value)으로 초기화 해봅시다. 다시 한 번 더 말하지만, 문자열은 포인터와 길이를 표현하는 두 워드(word)로 이루어진 데이터 구조입니다. 이 배열을 제로 값(zero value)으로 설정하면, 배열속의 모든 문자열도 제로 값(zero value)이 됩니다. 각각의 문자열의 첫 번째 워드는 nil을 가리키고 두 번째 워드는 0이 됩니다. 
...
여기서는 value의 의미로서 사용합니다. 문자열 값을 공유하지 않습니다. 
Println은 문자열의 값을 복사해서 가집니다. Println을 호출 할 때 같은 배열을 공유하는 4개의 문자열을 가지게 되는 것입니다. 문자열의 주소를 함수에 전달하지 않으면 이점이 있습니다. 문자열의 길이를 알고 있으니 스택에 둘 수 있고, 그 덕분에 힙에 할당하여 GC를 해야하는 부담을 덜게 됩니다. 
문자열은 값을 전달하여 스택에 둘 수 있게 디자인 되어, 가비지가 생성되지 않습니다. 그래서 문자열(들)이 가리키는 배열만이 힙에 저장되고 공유됩니다. 
