...
As soon as we add another Goroutine to our program, we add a huge amount of complexity. We can't always let the Goroutine run stateless. There has to be coordination. There are, in fact, 2 things that we can do with multi threaded software. - We either have to syncronize access to share state like that WaitGroup is done with Add, Done and Wait. - Or we have to coordinate these Goroutines to behave in a predictable or responsible manner.
...
The problem with data races is that they always appear random. Sample program to show ho to create race conditions in our programs. We don't want to do this.
...
The data races that we have here is that: at any given time, both Goroutines could be reading and writing at the same time. However, we are very lucky in this case. What we are seeing is that each Goroutine is executing the 3 statements atomically comletely by accident every time this code run.
...
Safely add one to the counter. Add the atomic functions that we have taken an address as the first parameter and that is being syncronized, no matter how many Goroutines they are. If we call one of these functions on the same location, they will get serialized. This the fastest way to serialization.
...
Only allow one Goroutine through this critical section at a time. Creating these artifical curly brakets gives readability. We don't have to do this but it is highly recommended. The Lock and Unlock function must always be together in line of sight.
...
There are times when we have a shared resources where we want many Goroutines reading it.
...
Number of reads occuring at any given time. As soon as we see int64 here, we should start thinking about using atomic instruction.
