...
`counter`에 안전하게 추가해줍니다. 주소를 첫번째 인자로 갖고, 얼마나 많은 고루틴이 있던지 상관없이 동기화 될 수 있는 아토믹 함수를 추가하십시오. 만약 우리가 같은 위치에서 여러개의 함수중 하나를 부른다면, 함수들은 직렬화됩니다. 이것은 직렬화하는 가장 빠른 방법입니다.

우리는 이 프로그램을 하루 종일 실행하더라도 매번 4라는 값을 얻을 수 있습니다.

```go
                atomic.AddInt64(&counter, 1)
```

이 호출은 이제 `AddInt64` 함수가 완료될 때까지 증가하기 때문에 관련이 없습니다.
