...
프로그램에 또 다른 고루틴을 추가하게 되면 복잡도가 올라가게 된다. 고루틴은 상태 없이(stateless) 실행할 수 없기 때문에 제어가 필요하고, 멀티 스레드 소프트웨어를 작성할 때 두 가지의 선택지가 있다.

- WaitGroup을 Add와 Done, Wait으로 제어하는 것 처럼, 공유 자원에 대한 접근 상태를 동기화해야 한다.
- 또는 고루틴들을 예측 가능하고, 계획한 대로 실행이 되도록 만들 수도 있다.

채널을 사용하기 전 까지, 원자성을 보장하는 기능인 mutex를 사용하여 앞서 언급한 두 가지 선택지를 구현한다. 채널은 간단한 제어 방법을 제공하지만, 대부분의 경우 mutex를 사용하여 공유 자원에 대한 액세스 동기화를 사용하는 것이 가장 좋은 방법이다.
