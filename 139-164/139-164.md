...
버퍼 없는 채널이 작동하는 방식은 다음과 같다. 채널에 신호를 주려는 고루틴이 있다고 해보자. 채널은 신호와 함께 데이터도 보내려고 한다. 고루틴은 채널에 바로 데이터를 넣을 것이다. 하지만 채널은 반대편에 신호를 받을 고루틴이 있는지 알아야 하기 때문에 그때까지 데이터는 잠기고(locked) 움직일 수 없게 된다. 결국 어떤 고루틴이 와서 데이터를 받겠다고 말한다. 두 고루틴 모두 채널에 관여하고 있지 않다.[재방문] 데이터는 이제 전송 가능하다.
