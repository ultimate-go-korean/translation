...
키는 타입 특정적이고(type specific), 동일한 타입의 값만 매치할 수 있다.
...
`Value`는 우리에게 적절하고 맞는 타입(the corrected type)을 전달할 수 있게 해 주고 (이 경우에 `userKey` 타입의 `uk`),
...
비록 키의 값이 `0` 이고, 우리가 이 함수 호출에 `0`을 전달한다고 해도
...
사용자 정의의 키 타입을 정의 해 보자.
이 방법을 통해서, 우리 혹은 이 타입에 대해서 인지하고 있는 사람만이 context에서 그 값을 꺼내 올 수 있다.
왜냐하면 만약, 다수의 일부 프로그램(multiple partial programs)이 값이 `0`인 것을 사용하고자 한다면,
모든 것이 엉망이 되어버릴 수 있다.
이 타입이 context 외부에서 값을 저장하고 검색할 수 있는 것에 대한 추가적인 보호 수준을 확장해 준다.
