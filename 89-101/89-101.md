...
Mocking은 중요하다. 네트워크 상에서 발생하는 대부분 것들이 mock 될 수 있다. 하지만, 데이터베이스를 mocking하는 것은 매우 복잡하기 때문에 mock 하기가 어렵다. 이때 Docker를 사용하여 테스트를 위해 새로운 데이터베이스를 생성함으로써 코드를 깔끔하게 할 수 있다.
