...
`admin` 은 특정 권한을 가진 관리자를 의미하는데, `person user` 는 임베딩이 아니다. 단지 `person` 이라는 필드는 `user` 라는 또다른 타입으로 정의되어 있고, 이 필드를 따로 생성하면 될 뿐이다.
...
`admin` 타입으로 만들어진 값은 `person` 필드를 통해 `notify` 를 호출할 수 있다.
...
`admin` 은 특정 권한을 가진 관리자를 의미하는데, 이번에는 `person` 필드를 어디에도 사용하지 않고, `admin` 타입 내부에 `user` 타입의 값을 임베딩할 것이다. 이는 `admin` 은 외부 타입(outer type), `user` 는 내부 타입(inner type) 으로 연관지은 inner-type-outer-type 관계다.
...
이제 외부 타입인 `admin` 과 내부 타입인 `user` 를 생성해보자. 여기서 내부 타입 값인 `user` 가 필드처럼 보이기는 하지만 이는 필드가 아니다. 필드처럼 타입명을 통해 내부 값에 접근할 수 있다. `user` 의 구조체 또한 리터럴을 통해 내부 값을 초기화 할 수 있다.
