...
오류 처리는 코드의 일부가 되는 것은 물론 로깅까지 이어진다. 로깅은 디버그를 하기 위함이다. 그래서 무엇이 일어났는지 파악할 수 있을 정도로만 행동을 취할 수 있는(actionable) 것들을 남기면 된다. 그 외 이상적인 것은 노이즈를 분석 지표에 따라 대시보드에 맞게 정형화하는 것이다. 예를 들어, 소켓의 연결과 끊어짐을 로그로 남길 수는 있지만 꼭 확인해야할 필요성은 없기에 행동을 취할 수 있지는 않다.
